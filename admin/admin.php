...
<script>
    $('.add_btn').on('click', function(){
        uni_modal("Add Admin", "manage_admin.php")
    })

    $('.edit_btn').on('click', function() {
        uni_modal("Edit Admin Details", "manage_admin.php?id="+$(this).data('id'))
    })
    ...
    $('.print_btn').on('click', function() {
        var wnd = window.open('prints/adminprint.php', '__blank', 'width=1000, height=800')
        setTimeout(() => {
            wnd.print()
            setTimeout(() => {
                wnd.close()
    ...
    $('.delete_btn').on('click', function(){
        $conf = confirm("Do you really want to delete admin")
        if ($conf == true) {
            $.ajax({
                url: '../process.php?p=delete_admin',
                method: 'POST',
